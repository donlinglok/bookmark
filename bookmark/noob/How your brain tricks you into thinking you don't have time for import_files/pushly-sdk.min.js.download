...
PushlySDK.watch('ready', () => {
    try {
        if (window.digitalData && 'page' in window.digitalData) {
            let p = {};
            p = Object.keys(window.digitalData['page']['category']).filter(key => ['primaryCategory', 'subCategory1', 'subCategory2'].indexOf(key) >= 0).reduce((obj2, key) => (p[key] = key + ": " + window.digitalData['page']['category'][key], p), {});
            if (Object.keys(p).length) { pushly('page_tag_visit', Object.values(p)) }
        }
    } catch (e) {}
});
